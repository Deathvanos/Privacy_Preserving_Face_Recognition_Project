...
def run_reconstruction(pca, noised_projection: Optional[np.ndarray]) -> List[Optional[str]]:
    ...
    for recon_flat in reconstructions:
        try:
            ...
            pil_img = image_numpy_to_pillow(recon_flat, resized_size=expected_shape)
...
def run_pipeline(
    ...
    if k_same_k_value is not None and k_same_k_value > 0:
        ...
        for subject_id, ordered_k_same_arrays in k_same_arrays_dict.items():
             ...
             for arr in ordered_k_same_arrays:
                 if arr is not None:
                     try:
                         pil_img = image_numpy_to_pillow(arr, resized_size=IMAGE_SIZE)
                         b64_img = image_pillow_to_bytes(pil_img)
    ...
    for subject_id in tqdm(image_groups, desc="Traitement PEEP par sujet"):
        ...
        try:
            ...
            peep_subject_output["mean_face_b64"] = image_pillow_to_bytes(image_numpy_to_pillow(mean_face, resized_size=IMAGE_SIZE))
    ...
    for subject_id, subject_preprocessed_list in image_groups.items():
        ...
        pipeline_result[subject_id] = {
            ...
            "resized": [image_pillow_to_bytes(img['resized_image']) if img.get('resized_image') else None for img in subject_preprocessed_list],
            "grayscale": [image_pillow_to_bytes(img['grayscale_image']) if img.get('grayscale_image') else None for img in subject_preprocessed_list],
