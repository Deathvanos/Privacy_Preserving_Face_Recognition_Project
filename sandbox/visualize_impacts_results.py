...
import base64
import io
import binascii # Pour gérer les erreurs de décodage base64
...
# Niveau DEBUG pour voir les messages ajoutés
...
def visualize_k_impact_line(subject_id_str: str, lfw_id: int, k_values: list[int],
    ...
    if variants_k_pil:
        save_path = os.path.join(output_dir, f"subject_{lfw_id}_2_k_impact.png")
...
def visualize_ratio_impact_line(subject_id_str: str, lfw_id: int, ratios: list[float], fixed_k: int,
    ...
    if variants_ratio_pil:
        save_path = os.path.join(output_dir, f"subject_{lfw_id}_3_ratio_impact_k{fixed_k}_no_noise.png")
        # Utilise l'originale prétraitée passée en argument
...
def visualize_epsilon_impact_line(subject_id_str: str, lfw_id: int, fixed_ratio: float, epsilons: list[float], fixed_k: int,
    ...
    if variants_eps_pil:
        save_path = os.path.join(output_dir, f"subject_{lfw_id}_4_epsilon_impact_k{fixed_k}_r{fixed_ratio:.2f}.png")
        # Utilise l'originale prétraitée passée en argument
...
def main():
    ...
    save_path_orig_lfw = os.path.join(PLOT_OUTPUT_DIR, f"subject_{lfw_id}_1_original_lfw_size.png")
    ...
    save_path_orig_proc = os.path.join(PLOT_OUTPUT_DIR, f"subject_{lfw_id}_1b_original_preprocessed.png")
